<template>
	<view class="culturedetail">
		...
		<view class="ping">
			...
			<view class="list">
				<view class="item" v-for="item in info?.Commentcen" :key="item.id">
					<view class="avator">
						<image src="@/static/picture/img.png" mode="" class="img"></image>
						<text>{{ item.name }}</text>
					</view>
		...
		<view class="fix">
			...
			<view class="sum">
				...
				<view class="item">
					...
					<text class="num">{{ info?.Commentcen.length }}</text>
				</view>
			</view>
		</view>
	</view>
...
<script setup>
	...
	import { centerbyidApi, addPingcenApi, addCenterZanApi } from '@/http/getinfo.js'
	...
	const getinfo = async (val) => {
		const { data:res } = await centerbyidApi(val)
		info.value = res.data
	}
	const addPing = async (data) => {
		const { data:res } = await addPingcenApi(data)
		utoast.value.show({
			title: '评论成功！',
			type: 'success',
			icon: false
	...
	// 点赞
	const addZan = async () => {
		const {data:res} = await addCenterZanApi(ids.value)
		info.value.zan ++
	}
