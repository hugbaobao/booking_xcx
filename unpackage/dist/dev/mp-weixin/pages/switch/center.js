...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const getinfo = async (val) => {
      const { data: res } = await http_getinfo.centerbyidApi(val);
      info.value = res.data;
    };
    const addPing = async (data) => {
      await http_getinfo.addPingcenApi(data);
      utoast.value.show({
        title: "评论成功！",
        type: "success",
        icon: false
    ...
    const addZan = async () => {
      await http_getinfo.addCenterZanApi(ids.value);
      info.value.zan++;
    };
    return (_ctx, _cache) => {
      ...
      return {
        ...
        i: common_vendor.f((_g = info.value) == null ? void 0 : _g.Commentcen, (item, k0, i0) => {
          return {
        ...
        q: common_vendor.t((_i = info.value) == null ? void 0 : _i.Commentcen.length),
        r: common_vendor.sr(utoast, "0f1e1aef-5", {
          "k": "utoast"
        })
      };
    };
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__scopeId", "data-v-0f1e1aef"], ["__file", "F:/单子/8.30门票预定/booking/pages/switch/center.vue"]]);
